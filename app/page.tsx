...
export default function Home() {
  ...
  return (
    <main className="relative">
      <img src="/images/illust/illus_2.jpg" className="fixed bottom-8 left-4 w-36 opacity-70 rotate-[-2deg] z-10 hover:opacity-90 transition-opacity duration-1000" alt="" />
      <img src="/images/illust/illus_3.jpg" className="fixed right-0 top-0 w-72 opacity-[0.06] z-0" alt="" />
      ...
      <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6">
          <h1 className="font-serif font-light text-4xl md:text-6xl text-white/80 leading-tight mb-6">これは、ただの霧ではありません。</h1>
      ...
      <section className="py-32 bg-background">
        <div className="max-w-6xl mx-auto px-6 space-y-32">
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="relative h-96"><img src="/images/forest_stream.png" className="w-full h-full object-cover" alt="" /></div>
            <div className="max-w-md">
              <p className="text-xs tracking-[0.2em] text-accent-rust mb-4">資産01 — 自然</p>
              <h2 className="font-serif font-light text-3xl md:text-4xl leading-relaxed text-foreground">
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div className="order-1 md:order-2 relative h-96"><img src="/images/food_kaiseki.png" className="w-full h-full object-cover" alt="" /></div>
          </div>
          <div className="grid md:grid-cols-2 gap-16 items-center">
            <div className="relative h-96"><img src="/images/craftsman_hands.png" className="w-full h-full object-cover" alt="" /></div>
            <div className="max-w-md">
      ...
      {/* 資産の解像度 */}
      <section className="py-24 bg-background"><p className="text-center text-xs tracking-[0.2em] text-accent-rust mb-16">資産の解像度</p><div className="max-w-6xl mx-auto px-6 grid grid-cols-2 gap-6"><div className="space-y-6"><div className="h-64 w-full"><img src="/images/detail_steam.png" className="w-full h-full object-cover" alt="" /></div><div className="h-80 w-full"><img src="/images/detail_wood.png" className="w-full h-full object-cover" alt="" /></div><div className="h-72 w-full"><img src="/images/detail_light.png" className="w-full h-full object-cover" alt="" /></div></div><div className="space-y-6 mt-24"><div className="h-72 w-full"><img src="/images/detail_hand.png" className="w-full h-full object-cover" alt="" /></div><div className="h-64 w-full"><img src="/images/detail_water.png" className="w-full h-full object-cover" alt="" /></div><div className="h-80 w-full"><img src="/images/detail_stone.png" className="w-full h-full object-cover" alt="" /></div></div></div></section>
      ...
      {/* Act 3: サービスカード */}
      <section ref={serviceCardsRef} className="py-24 bg-background"><div className="max-w-6xl mx-auto px-6"><h2 className="text-center font-serif font-light text-3xl mb-16 text-foreground">私たちの視点</h2><div className="grid grid-cols-1 md:grid-cols-5 gap-8">{[ { title: 'グローバルOTA運用', desc: '世界77カ国の視点で、あなたの宿を最適な販路へ。' }, { title: 'データドリブン単価最適化', desc: '感覚を数値に。適正価格が、最高の利益を呼ぶ。' }, { title: '自社予約シフト設計', desc: 'リピーターを育み、OTA手数料から卒業する仕組み。' }, { title: '多言語ブランド構築', desc: '日・英・中、3言語で紡ぐ、世界に響く物語。' }, { title: '越境エコシステム', desc: '教育、村づくり、EC。集客のその先まで、共に。' }].map((s, i) => (<div key={i} className="text-center md:text-left p-6"><h3 className="font-serif font-light text-lg mb-3 text-foreground">{s.title}</h3><p className="text-sm text-foreground/70 leading-relaxed">{s.desc}</p></div>))}</div></div></section>

      {/* 予告カード */}
      <section className="py-16 bg-background"><div className="max-w-2xl mx-auto px-6 text-center border border-hairline p-8"><h3 className="font-serif font-light text-2xl text-foreground mb-4">では、あなたの宿は、どれくらい伸びるのか。</h3><p className="text-sm text-foreground/60 mb-6">稼働率とお部屋単価を動かすだけで、3秒でわかります。</p><button onClick={openSelector} className="bg-[#ABBAA9] text-black font-['Zen_Old_Mincho'] px-6 py-2 text-sm tracking-widest hover:bg-[#B4BC4E] hover:text-white transition-colors">宿の伸びしろを測定</button></div></section>

      {/* 水平スクロール・ポートフォリオ */}
      <section className="py-24 bg-background">
        <h2 className="text-center font-serif font-light text-3xl mb-16 text-foreground">私たちは、このように宿を見ています</h2>
      ...
      <section className="py-24 bg-background relative overflow-hidden"><img src="/images/illust/illus_1.jpg" className="absolute top-20 left-[-40px] w-40 opacity-40 rotate-[-3deg] z-0 hover:opacity-80 transition-opacity duration-1000" alt="" /><img src="/images/illust/illus_2.jpg" className="absolute bottom-40 right-[-30px] w-48 opacity-30 rotate-[2deg] z-0 hover:opacity-80 transition-opacity duration-1000" alt="" />
        <div className="max-w-6xl mx-auto px-6 relative z-10"><div className="text-center mb-24"><p className="text-xs tracking-[0.2em] text-accent-rust mb-4">OUR METHOD</p>
          <h2 className="font-serif font-light text-3xl md:text-5xl text-foreground mb-6">私たちは「モノのないデザイン」を届けます。</h2>
          <p className="text-sm text-foreground/60 max-w-2xl mx-auto leading-relaxed mb-8">日本各地の宿や地域に眠る価値を再発見し、「日本だけでなく世界中から訪れたくなる目的地」へと再設計する。それが、私たちのデスティネーションデザインです。</p>
          <p className="font-['Ogg',_'Times_New_Roman',_serif] font-light text-2xl md:text-4xl text-[#6C6059] tracking-[0.15em] mt-8">A Stay Starts a Story</p></div><div className="space-y-32">{[ { number:"01", title:"Meaning Design", subtitle:"意味の再定義", desc:"既存の価値観や前提を根底から覆し、「本来の意味」を再発見して魂を入れるフェーズ。", img:"/images/method_meaning.jpg" }, { number:"02", title:"Innovation Design", subtitle:"パラダイムの設計", desc:"見出した意味をベースに、全く新しい顧客体験や「届けるべき相手」の枠組みをデザイン。", img:"/images/method_innovation.jpg" }, { number:"03", title:"Business Design", subtitle:"収益構造の設計", desc:"アイデアで終わらせず、価格設計や販路（直販化）など、確実に稼ぎ続けるビジネスモデルを構築。", img:"/images/method_business.jpg" }, { number:"04", title:"Business Transformation", subtitle:"変革の実装", desc:"組織、採用体制、システム、顧客体験のすべてを根本から変容させ、現実のエコシステムを動かします。", img:"/images/method_transformation.jpg" }].map((m, i) => (<div key={i} className="grid md:grid-cols-2 gap-16 items-center"><div className={`relative h-96 ${i%2===1 ? "md:order-2" : ""}`}><img src={m.img} className="w-full h-full object-cover shadow-2xl" alt={m.title} /><img src={`/images/illust/illus_${i+3}.jpg`} className="absolute -bottom-6 -right-6 w-24 opacity-60 rotate-[4deg] hover:opacity-100 transition-opacity duration-700 z-10" alt="" /></div><div className={`max-w-md ${i%2===1 ? "md:order-1" : ""}`}><p className="text-6xl font-light text-accent-rust/20 mb-4">{m.number}</p><h3 className="font-serif font-light text-2xl md:text-3xl text-foreground mb-2">{m.title}</h3><p className="text-xs tracking-[0.2em] text-accent-rust mb-6">{m.subtitle}</p><p className="text-sm text-foreground/60 leading-relaxed">{m.desc}</p></div></div>))}</div></div>
      </section>

      {/* 4つのステップ */}
      <section className="py-24 bg-background relative overflow-hidden border-t border-hairline"><img src="/images/illust/illus_5.jpg" className="absolute top-1/2 left-[10%] w-36 opacity-30 rotate-[-2deg] z-0 hover:opacity-80 transition-opacity duration-1000" alt="" /><img src="/images/illust/illus_6.jpg" className="absolute top-20 right-[5%] w-44 opacity-25 rotate-[3deg] z-0 hover:opacity-80 transition-opacity duration-1000" alt="" /><div className="max-w-6xl mx-auto px-6 relative z-10"><div className="text-center mb-24"><p className="text-xs tracking-[0.2em] text-accent-rust mb-4">OUR PROCESS</p><h2 className="font-serif font-light text-3xl md:text-5xl text-foreground mb-6">利益を再投資し、共に成長する。</h2><p className="text-sm text-foreground/60 max-w-2xl mx-auto leading-relaxed">共に成果を生み出しながら、4つのステップで確実に変革を実装します。</p></div><div className="space-y-32">{[ { step:"Step 1", title:"足がかり", subtitle:"現場の最適化と利益創出", desc:"OTA運用代行や既存事業の運営支援から入り、現場の無駄を省いて即効性のある利益を生み出します。", img:"/images/step1_foundation.jpg" }, { step:"Step 2", title:"攻めの変革", subtitle:"Web・ダイレクト戦略の構築", desc:"生み出した利益を原資に、自社Webサイト作成。脱下請け・脱OTAを実現し、直接顧客と繋がる強い構造を作ります。", img:"/images/step2_offense.jpg" }, { step:"Step 3", title:"守りの変革", subtitle:"HR BPO・組織の再構築", desc:"新たなステージに合わせて組織を最適化。人事戦略家（HRBPO）として、無駄な採用費を削り、定着率を高め、盤石な組織を作ります。", img:"/images/step3_defense.jpg" }, { step:"Step 4", title:"プロジェクト完了・自走化", subtitle:"顧問パートナーへ", desc:"攻めと守りの完璧な動線が完成したらプロジェクト完了。その後もサステナブルな成長を目指し、経営・人事の社外アドバイザーとして共創し続けます。", img:"/images/step4_autonomy.jpg" }].map((st, i) => (<div key={i} className="grid md:grid-cols-2 gap-16 items-center"><div className={`relative h-96 ${i%2===1 ? "md:order-2" : ""}`}><img src={st.img} className="w-full h-full object-cover shadow-2xl" alt={st.title} />{i===3 && <img src="/images/illust/illus_7.jpg" className="absolute -top-6 -left-6 w-28 opacity-70 rotate-[-6deg] hover:opacity-100 transition-opacity duration-700 z-10" alt="" />}</div><div className={`max-w-md ${i%2===1 ? "md:order-1" : ""}`}><p className="text-sm tracking-[0.2em] text-accent-rust mb-2">{st.step}</p><h3 className="font-serif font-light text-2xl md:text-3xl text-foreground mb-2">{st.title}</h3><p className="text-xs tracking-[0.2em] text-accent-rust mb-6">{st.subtitle}</p><p className="text-sm text-foreground/60 leading-relaxed">{st.desc}</p></div></div>))}</div></div></section>

      {/* FAQ */}
      <section className="py-24 bg-background border-t border-hairline"><div className="max-w-3xl mx-auto px-6"><div className="text-center mb-16"><p className="text-xs tracking-[0.2em] text-accent-rust mb-4">FAQ</p><h2 className="font-serif font-light text-3xl text-foreground">よくある質問</h2></div><div className="space-y-8">{[ { q: "本当に初月は完全無料ですか？", a: "はい。まずは1ヶ月、完全無料でOTAの基礎設定や分析を行います。その間、一切の費用は発生しません。私たちがまず価値を証明します。" }, { q: "どのような宿でも対応可能ですか？", a: "旅館、ホテル、民宿、ゲストハウスまで、規模や形態を問わず対応します。特に地方の中小規模の宿泊施設の課題に寄り添います。" }, { q: "費用はかかりますか？", a: "初期費用はいただいておりません。初月は完全無料でご利用いたせます。2ヶ月目以降は成果報酬制度となります。" }, { q: "他社のOTA代行と何が違うのか？", a: "海外でのネットワークや経験値が圧倒的に異なります。私たちは、OTAを上手に使いこなすだけの代行業ではなく、新たなマーケットからの新規売上獲得を共創する「デスティネーションデザイン」を行います。" }, { q: "診断だけでも利用できますか？", a: "はい、このサイトから無料で、どなたでもご利用いただけます。詳細な分析をご希望の場合は、コンサルタントとのオンラインミーティングをご案内しています。" }].map((faq, i) => (<details key={i} className="group border-b border-hairline pb-6"><summary className="flex justify-between items-center cursor-pointer list-none"><span className="font-serif font-light text-lg text-foreground">{faq.q}</span><span className="text-accent-rust text-2xl group-open:hidden">+</span><span className="text-accent-rust text-2xl hidden group-open:inline">-</span></summary><p className="mt-4 text-sm text-foreground/60 leading-relaxed">{faq.a}</p></details>))}</div></div></section>

      {/* 中間CTA */}
      <section className="py-32 bg-background text-center border-t border-hairline relative overflow-hidden"><img src="/images/illust/illus_1.jpg" className="absolute top-0 left-1/2 -translate-x-1/2 w-96 opacity-20 rotate-[-2deg] z-0" alt="" /><div className="relative z-10 max-w-2xl mx-auto px-6"><h2 className="font-serif font-light text-3xl md:text-4xl text-foreground mb-8">まずは、あなたの宿の「眠れる資産」を教えてください。</h2><p className="text-sm text-foreground/60 mb-12 leading-relaxed">私たちが最初にやることは、無料であなたの宿の可能性を診断することです。リスクはゼロ。そこから、あなたと共に次の一歩を考えます。</p><a href="https://timerex.net/s/InnBuddy_ESG/386fd946" target="_blank" rel="noopener noreferrer" className="inline-block bg-[#ABBAA9] text-black font-['Zen_Old_Mincho'] px-10 py-4 text-sm tracking-widest hover:bg-[#B4BC4E] hover:text-white transition-colors duration-700">お話を聞かせてください</a></div></section>

      {/* 呼吸モジュール② */}
      <section className="py-16 bg-background">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <img src="/images/breath_4.jpg" className="h-48 w-full object-cover" alt="" />
            <img src="/images/breath_5.jpg" className="h-48 w-full object-cover" alt="" />
            <img src="/images/breath_6.jpg" className="h-48 w-full object-cover" alt="" />
            <img src="/images/breath_7.jpg" className="h-48 w-full object-cover" alt="" />
            <img src="/images/breath_8.jpg" className="h-48 w-full object-cover" alt="" />
          </div>
      ...
      <section className="relative py-32 bg-background overflow-hidden">
        <div className="relative max-w-4xl mx-auto h-[600px]">
          <img src="/images/guest_peace.jpg" className="absolute top-4 left-[28%] z-20 rotate-[-2deg] w-48 h-64 object-cover border-4 border-white shadow-sm" alt="" />
          <img src="/images/local_smile.jpg" className="absolute top-16 left-[48%] z-10 rotate-[1.5deg] w-48 h-64 object-cover border-4 border-white shadow-sm" alt="" />
          <img src="/images/entrance_night.jpg" className="absolute top-8 left-[62%] z-30 rotate-[-1deg] w-48 h-64 object-cover border-4 border-white shadow-sm" alt="" />
          <img src="/images/kids_future.jpg" className="absolute top-24 left-[32%] z-0 rotate-[2.5deg] w-48 h-64 object-cover border-4 border-white shadow-sm" alt="" />
          <img src="/images/photo_essay_5.jpg" className="absolute top-0 left-[42%] z-15 rotate-[-3.5deg] w-44 h-60 object-cover border-4 border-white shadow-sm" alt="" />
          <img src="/images/photo_essay_6.jpg" className="absolute top-32 left-[52%] z-5 rotate-[0.5deg] w-48 h-64 object-cover border-4 border-white shadow-sm" alt="" />
          <img src="/images/photo_essay_7.jpg" className="absolute top-12 left-[22%] z-25 rotate-[-0.5deg] w-44 h-60 object-cover border-4 border-white shadow-sm" alt="" />
          <div className="absolute top-[65%] left-1/2 -translate-x-1/2 z-40 bg-background/80 backdrop-blur-sm px-8 py-6">
      ...
      <section className="py-24 bg-background">
        <div className="max-w-4xl mx-auto px-6">
          ...
          <p className="text-center font-serif text-xl md:text-2xl text-foreground leading-relaxed max-w-4xl mx-auto">
      ...
      <section className="relative py-48 bg-background">
        ...
        <div className="relative z-10 text-center px-6 max-w-2xl mx-auto">
          <h2 className="font-serif font-light text-3xl md:text-5xl text-white mb-8 leading-tight whitespace-nowrap">あなたの物語を、世界の感動へ。</h2>
          <p className="text-white/80 mb-12 font-sans">まずは、御宿の“眠れる資産”を診断するところから。</p>
          <a href="https://timerex.net/s/InnBuddy_ESG/386fd946" target="_blank" rel="noopener noreferrer" className="inline-block bg-[#FDF7F5] text-[#515D46] font-['Zen_Old_Mincho'] px-10 py-4 text-sm tracking-widest hover:bg-[#B4BC4E] hover:text-white transition-colors duration-700">無料相談を予約する</a>
          <p className="mt-8 text-xs text-white/40 tracking-widest">InnBuddy — 越境エコシステムの入り口</p>
        </div>
      </section>

      {/* 診断入口カード（オリーブベース＋黄色グラデーション） */}
      <section ref={diagnosticSectionRef} className="py-24 bg-background border-t border-hairline">
        <div className="max-w-6xl mx-auto px-6 grid md:grid-cols-2 gap-8">
          <div className="relative overflow-hidden rounded-sm shadow-md border border-[var(--diag-brown1)]/20 p-8 text-center">
