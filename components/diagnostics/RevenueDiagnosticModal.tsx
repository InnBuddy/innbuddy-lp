...
export default function RevenueDiagnosticModal({ isOpen, onClose }: Props) {
  ...
  const toggleOta = (ota: string) => {
    setSelectedOta(prev =>
      prev.includes(ota) ? prev.filter(o => o !== ota) : prev.length < 3 ? [...prev, ota] : prev
    );
  };
  ...
  const currentAnnualRevenue = totalRooms
    ? parseInt(totalRooms) * 365 * (currentOCC / 100) * currentADR
    : 0;
  const newAnnualRevenue = totalRooms
    ? parseInt(totalRooms) * 365 * (newOCC / 100) * newADR
    : 0;
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="relative bg-background w-full max-w-2xl max-h-[90vh] overflow-y-auto p-8 md:p-12 rounded-sm shadow-xl overflow-hidden">
        ...
        <div className="relative z-10">
          <button onClick={handleBack} className="absolute top-4 left-4 text-sm text-foreground/50 hover:text-foreground transition-colors">
            ← 前のページに戻る
          </button>
          ...
          {!showResult ? (
            <div className="space-y-8 pt-8">
              ...
              <div>
                ...
                <select
                  value={primaryOta}
                  onChange={e => setPrimaryOta(e.target.value)}
                  className="w-full border-b border-hairline bg-transparent py-2 focus:outline-none focus:border-accent-rust"
                >
              ...
              <div>
                ...
                <input
                  type="number"
                  value={primaryOtaShare}
                  onChange={e => setPrimaryOtaShare(e.target.value)}
                  onWheel={e => (e.target as HTMLElement).blur()}
                  className="w-full border-b border-hairline bg-transparent py-2 focus:outline-none focus:border-accent-rust"
                />
              </div>
              <div className="pt-6 border-t border-hairline">
                ...
                <input
                  ...
                  step="100"
                  value={adrUp}
                  onChange={e => setAdrUp(parseInt(e.target.value))}
                  className="w-full accent-accent-rust"
                />
                ...
                <input
                  ...
                  step="1"
                  value={occUp}
                  onChange={e => setOccUp(parseInt(e.target.value))}
                  className="w-full accent-accent-rust"
                />
              </div>
              <button
                onClick={handleSubmit}
                className="w-full bg-[var(--diag-accent)] text-white font-['Zen_Old_Mincho'] py-3 text-sm tracking-widest hover:opacity-90 transition-opacity"
              >
                シミュレーション結果を見る
              </button>
            </div>
            ...
            <div className="text-center pt-8">
              <div className="py-12 space-y-6">
                <p className="text-6xl font-light" style={{ color: 'var(--diag-accent)' }}>
                  {formattedUpAmount}円
                </p>
