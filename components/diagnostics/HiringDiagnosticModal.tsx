  ...
  return (
    <div>
      ...
      <input
        ...
        onBlur={onBlur}
        className={`w-full border-b bg-transparent py-2 focus:outline-none focus:border-accent-rust ${
  ...
  return (
    <div className="space-y-6">
      ...
      <InputField
        ...
        onChange={(v) => { updateAnswer('employeeCount', v); clearError('employeeCount'); }}
        hasError={errors.employeeCount}
      />
      <InputField
        label="契約社員数"
        value={answers.contractCount}
        onChange={(v) => updateAnswer('contractCount', v)}
      />
      <InputField
        label="パート・アルバイト数"
        value={answers.partTimeCount}
        onChange={(v) => updateAnswer('partTimeCount', v)}
      />
      <InputField
        label="外国人材数"
        value={answers.foreignCount}
        onChange={(v) => updateAnswer('foreignCount', v)}
      />
      <div>
        <label className="text-sm text-foreground/70">年間採用予算</label>
        <select
          value={typeof answers.budget === 'string' ? answers.budget : ''}
          onChange={(e) => updateAnswer('budget', e.target.value)}
          className="w-full border-b border-hairline bg-transparent py-2"
        >
      ...
      <InputField
        ...
        onChange={(v) => updateAnswer('turnover', v)}
      />
      <button
        onClick={onNext}
        className="w-full bg-[#ABBAA9] text-black font-['Zen_Old_Mincho'] py-3 text-sm tracking-widest hover:bg-[#B4BC4E] hover:text-white transition-colors mt-8"
      >
        次へ
      </button>
    </div>
  ...
  return (
    <div className="space-y-8">
      ...
      {hiringScoreQuestions.map((q: ScoringQuestion) => (
        <div
          key={q.id}
          className={`p-3 rounded-sm transition-colors ${errors[q.id] ? 'border-2 border-red-500' : 'border-2 border-transparent'}`}
        >
          ...
          <div className="flex gap-2 flex-wrap">
            {q.options.map((opt: string, idx: number) => (
              <button
                key={idx}
                onClick={() => { updateAnswer(q.id, idx); clearError(q.id); }}
      ...
      <div className="flex justify-between mt-8">
        <button onClick={onPrev} className="text-sm text-foreground/50 hover:text-foreground transition-colors">
          ← 前のステップへ
        </button>
        <button
          onClick={onNext}
          className="bg-[#ABBAA9] text-black font-['Zen_Old_Mincho'] py-3 px-8 text-sm tracking-widest hover:bg-[#B4BC4E] hover:text-white transition-colors"
        >
          次へ
        </button>
      </div>
        ...
        >
          ...
          <div className="flex gap-2 flex-wrap">
            {q.options.map((opt: string, idx: number) => (
              <button
                key={idx}
                onClick={() => { updateAnswer(q.id, opt); clearError(q.id); }}
                className={`px-3 py-1 text-xs border transition-all ${
                  answers[q.id] === opt
                    ? 'border-foreground bg-black/5 text-black font-semibold'
                    : 'border-hairline text-foreground/60'
...
function ResultScreen({ answers, onClose }: { answers: Record<string, any>; onClose: () => void }) {
  ...
  return (
    <div className="text-center space-y-6">
      ...
      {score < 40 && (
        <span className="inline-block px-3 py-1 text-xs rounded-full text-white" style={{ backgroundColor: 'var(--diag-red)' }}>警告</span>
      )}
...
export default function HiringDiagnosticModal({ isOpen, onClose }: ModalProps) {
  ...
  const handleNext = () => {
    if (step === 1) {
      if (answers.employeeCount !== undefined) {
        ...
        setErrors({ employeeCount: true });
      }
    } else if (step === 2) {
      const newErrors: Record<string, boolean> = {};
      hiringScoreQuestions.forEach(q => {
        if (answers[q.id] === undefined) newErrors[q.id] = true;
      });
  ...
  // ★ 「← 前のページに戻る」ボタンの動作を動的に決定
  const handleMainBack = () => {
    if (step === 1) {
      onClose(); // Step1ならLPに戻る
    } else {
      handlePrev(); // Step2以降なら前のステップへ
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="relative bg-background w-full max-w-2xl max-h-[90vh] overflow-y-auto p-8 md:p-12 rounded-sm shadow-xl overflow-hidden">
        <DiagnosticBackground showContourOverlay={true} />
        <div className="relative z-10">
          {/* ★ 動的に動作が変わる「戻る」ボタン */}
          <button onClick={handleMainBack} className="absolute top-4 left-4 text-sm text-foreground/50 hover:text-foreground transition-colors">
            ← 前のページに戻る
          </button>
